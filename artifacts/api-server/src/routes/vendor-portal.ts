...
function cSet(k: string, ttl: number, d: unknown) {
  _cache.set(k, { data: d, exp: Date.now() + ttl, setAt: Date.now() });
}
...
const DASH_MAX_STALE_MS = 2 * 60_000; // never serve dashboard data older than this
  ...
  const [routerRow, [totalsRows, byProfileRaw, [periodStatsRow], recentSales, availableVouchers, validProfileRows]] = await Promise.all([
    ...
    Promise.all([
      ...
      db.select().from(vouchersTable).where(and(
        ...
        gte(
          sql`coalesce(${vouchersTable.usedAt}, ${vouchersTable.printedAt})`,
