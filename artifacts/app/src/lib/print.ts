 ...
 *
 * autoPrint=true  : même stratégie que le mode mobile normal (zoom+table+tr).
 *   - zoom sur <html> (pas transform:scale qui casse le rendu print iOS)
 *   - tickets dans <table><tr> : page-break-inside:avoid sur <tr> est le seul
 *     sélecteur fiable sur iOS Safari (inline-block/flex sont ignorés).
 *   - page-break-after:always entre blocs de page (pré-calculés) : pas de
 *     dépendance au moteur de pagination iOS pour les sauts de page.
 * autoPrint=false : barre interactive avec sélecteur d'échelle (écran seul).
 */
export function buildSmallModePrintHtml(htmlItems: string[], title: string, defaultScale = 0.85, autoPrint = false): string {

  // ── CHEMIN autoPrint : stratégie table+tr (anti-coupure iOS) ─────────────
  if (autoPrint) {
    const COLS = 4;          // tickets par rangée (même défaut que mode mobile)
    const ROWS_PER_PAGE = 9; // rangées par bloc de page
    ...
    for (let p = 0; p < htmlItems.length; p += perPage) {
      ...
      const breakStyle = isLast ? "" : `page-break-after:always;break-after:page;`;
      blocks.push(
        `<div class="ticket-page-wrap" style="${breakStyle}">` +
        `<table class="ticket-page"><tbody>${rows.join("")}</tbody></table></div>`
      );
    }

    // @page DOIT être à la racine (Safari iOS l'ignore dans @media print)
    const pageCss = `
      @page        { margin: 9mm 3mm 3mm 7mm; }
      @page :first { margin: 9mm 3mm 3mm 7mm; }
      @page :left  { margin: 9mm 3mm 3mm 7mm; }
      @page :right { margin: 9mm 3mm 3mm 7mm; }
    `;

    const printCss = `
      html { zoom: ${defaultScale}; margin: 0; padding: 0; }
      html, body { margin: 0; padding: 0; }
      body {
        color: #000; background: #fff;
        font-size: 14px; font-family: Helvetica, Arial, sans-serif;
      ...
      table.ticket-page { border-collapse: collapse; margin: 0; }
      table.ticket-page > tbody > tr > td { padding: 2px; vertical-align: top; }

      /* Seule règle anti-coupure fiable sur iOS Safari : page-break-inside sur tr */
      .ticket-row { page-break-inside: avoid; break-inside: avoid; }

      /* Apparence ticket MikHmon Small */
      table.voucher {
      ...
      /* overflow:visible sur conteneurs (fix Safari page-break),
         overflow:hidden maintenu sur table.voucher (triangle décoratif) */
      body, .ticket-page-wrap, table.ticket-page, .ticket-row, .ticket-row > td {
        overflow: visible !important;
      }

      /* Numéro MikHmon */
      span#num, #num { float: right !important; margin-left: 4px !important; clear: none !important; }
...
<body>${blocks.join("")}</body>
  ...
  // ── CHEMIN INTERACTIF (barre de contrôle, écran seul) ────────────────────
  const css = `
    @page {
      size: auto;
      margin-left: 7mm;
